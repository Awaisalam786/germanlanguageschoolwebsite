...
export async function POST(req) {
  try {
    ...
    // If vocab_engine_attempts lacks user_id column in database before migration is applied,
    ...
    if (error && error.message && error.message.includes('user_id')) {
      delete row.user_id;
      const retry = await supabase.from('vocab_engine_attempts').insert([row]);
      data = retry.data;
      error = retry.error;
    }

    if (error) {
      console.error('[save-vocab-attempt] Insert error:', error.message);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    return NextResponse.json({ success: true, data });
  } catch (error) {
    console.error('[save-vocab-attempt] Server error:', error);
