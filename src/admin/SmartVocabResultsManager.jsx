...
export default function SmartVocabResultsManager() {
  ...
  const fetchResults = async () => {
    ...
    if (error) {
      console.error(error);
      if (error.code === '42P01') {
        alert("Table 'vocab_engine_attempts' does not exist. Please run the SQL schema script.");
      }
    } else {
      setResults(data || []);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-extrabold text-white">Smart Vocab Results</h1>
          <p className="text-slate-400 text-sm mt-1">Detailed scores and word-by-word breakdowns from the Chapter Engine.</p>
        </div>
                    ...
                    {expandedRows.has(r.id) && (
                      <tr>
                        <td colSpan="6" className="p-0 border-b border-slate-800 bg-slate-950">
                          <div className="p-6">
                            <h4 className="text-sm font-bold text-slate-300 mb-4 border-b border-slate-800 pb-2">Word-by-Word Breakdown</h4>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                              {r.word_results && r.word_results.map((word, idx) => (
                                <div key={idx} className={`p-3 rounded-lg border ${word.isCorrect ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-red-500/5 border-red-500/20'}`}>
                                  <div className="flex justify-between items-start mb-1">
                                    <span className="text-xs text-slate-400 font-bold">{word.english}</span>
                                    {word.isCorrect ? <CheckCircle className="w-4 h-4 text-emerald-500" /> : <XCircle className="w-4 h-4 text-red-500" />}
                                  </div>
                                  <p className="text-sm font-bold text-white mb-1">{word.german}</p>
                                  <p className="text-[10px] text-slate-500">
                                    User answered: <span className={word.isCorrect ? 'text-emerald-400' : 'text-red-400'}>
                                      {word.userAnswer || '(Skipped)'}
                                    </span>
                                  </p>
                                </div>
                              ))}
