...
import { Loader2, RotateCcw, CheckCircle, XCircle, ChevronLeft, AlertCircle } from 'lucide-react';
...
export default function ReadingTestEngine({
  ...
  const handleSubmit = async () => {
    ...
    await executeSave(currentScore, totalMarks, currentPercentage);
  };

  const executeSave = async (currentScore, currentTotalMarks, currentPercentage) => {
    setSaving(true);
    setSaveError(null);
    ...
    if (userType === 'student') {
      ...
      payload.user_id = storedFreeUser?.id || null;
    }
    ...
    try {
      ...
      const res = await fetch('/api/save-reading-attempt', {
        method: 'POST',
        headers,
          ...
          <div className="bg-slate-900 border border-rose-500/30 rounded-3xl p-6 shadow-xl text-left space-y-3">
            <p className="text-sm font-semibold text-rose-300">
              {userType === 'student'
                ...
                : 'Your reading test score could not be saved to your account.'}
            </p>
            <p className="text-xs text-slate-400 bg-slate-950 p-3 rounded-xl border border-slate-800 font-mono break-words">
              {saveError}
            </p>
            <p className="text-xs text-slate-400">
              Completed score: <strong className="text-white">{score} / {totalMarks}</strong> ({percentage}%)
            </p>
          </div>
          <div className="flex flex-col sm:flex-row justify-center gap-3">
            <button
              onClick={() => executeSave(score, totalMarks, percentage)}
